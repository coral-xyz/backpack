...
SELECT users.id AS user_id,
    rank() OVER (ORDER BY (COALESCE(mad_feed_user_promo_results.promotion_results_total, (0)::numeric)+COALESCE(mad_feed_user_promo_post_sums.promoted_total, (0)::numeric) + COALESCE(sum(mad_feed_post_vote_sums.votes), (0)::numeric)) DESC) AS rank,
    (COALESCE(mad_feed_user_promo_results.promotion_results_total, (0)::numeric)+COALESCE(mad_feed_user_promo_post_sums.promoted_total, (0)::numeric) + COALESCE(sum(mad_feed_post_vote_sums.votes), (0)::numeric)) AS total_score,
