...
export class Provider {
    ...
    _handleRpcResponse(event) {
        if (event.data.type !== CHANNEL_RPC_RESPONSE)
            return;
    ...
    _handleNotification(event) {
        ...
        if (event.data.type !== CHANNEL_NOTIFICATION)
            return;
    ...
    async request({ method, params }) {
        ...
        window.postMessage({ type: CHANNEL_RPC_REQUEST, detail: { id, method, params } }, POST_MESSAGE_ORIGIN);
        return await prom;
    }
