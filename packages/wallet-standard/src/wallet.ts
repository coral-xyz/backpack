...
export class BackpackWallet implements Wallet {
  ...
  #signAndSendTransaction: SolanaSignAndSendTransactionMethod = async (
    ...
    if (inputs.length === 1) {
      ...
      const connection =
        getChainForEndpoint(window.backpack.connection.rpcEndpoint) ===
        input.chain
          ? undefined
          : new Connection(
              ...
              commitment ||
                preflightCommitment ||
                window.backpack.connection.commitment
            );
  ...
  #signTransaction: SolanaSignTransactionMethod = async (...inputs) => {
    ...
    if (inputs.length === 1) {
      ...
      const transaction = VersionedTransaction.deserialize(input.transaction);
      const publicKey = new PublicKey(input.account.publicKey);
      const signedTransaction = await window.backpack.signTransaction(
        transaction,
        publicKey
      );
  ...
  #signMessage: SignMessageMethod = async (...inputs) => {
    ...
    if (inputs.length === 1) {
      ...
      const signature = await window.backpack.signMessage(
        signedMessage,
        publicKey
      );
