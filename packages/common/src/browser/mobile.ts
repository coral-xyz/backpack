...
export function startMobileIfNeeded() {
  ...
  BrowserRuntimeCommon.getSessionStorage = async (
    key: string
  ): Promise<any> => {
    const { result }: Response = await BackendRequestManager.request({
      channel: MOBILE_CHANNEL_HOST_RPC_REQUEST,
  ...
  BrowserRuntimeCommon.removeSessionStorage = async (
    key: string
  ): Promise<void> => {
    // safety precaution
    // session password never get clears
    // const { result }: Response = await BackendRequestManager.request({
    //   channel: MOBILE_CHANNEL_HOST_RPC_REQUEST,
    //   data: {
    //     id: generateUniqueId(),
  ...
  BrowserRuntimeCommon.setSessionStorage = async (
    key: string,
    value: any
  ): Promise<void> => {
    const { result }: Response = await BackendRequestManager.request({
  ...
  BrowserRuntimeCommon.clearSessionStorage = async (
    keys: string[]
  ): Promise<void> => {
    const { result }: Response = await BackendRequestManager.request({
      channel: MOBILE_CHANNEL_HOST_RPC_REQUEST,
  ...
  const handleHostRpcRequest = async ({
    ...
    switch (method) {
      ...
      case "getSessionStorage":
        return await handleGetSessionStorage(params[0]);
      case "removeSessionStorage":
        return await handleRemoveSessionStorage(params[0]);
      case "setSessionStorage":
        return await handleSetSessionStorage(params[0], params[1]);
      case "clearSessionStorage":
        return await handleClearSessionStorage(params[0]);
      default:
        return [];
    }
  };
  ...
  const handleGetLocalStorage = async (key: string): Promise<[any, any]> => {
    try {
  ...
  const handleRemoveSessionStorage = async (key: string) => {
    try {
  ...
  const handleGetSessionStorage = async (key: string): Promise<[any, any]> => {
    try {
  ...
  const handleSetSessionStorage = async (
    key: string,
    value: any
  ): Promise<[string, any]> => {
  ...
  const handleClearSessionStorage = async (keys: string[]) => {
    try {
      await SecureStorage.clear(keys);
      return ["success", undefined];
    } catch (error) {
      return ["error", error];
    }
