...
import type { SolanaFeeConfig } from "@coral-xyz/secure-background/types";
import {
...
function useSolanaTxData(
    ...
    if (transaction) {
      ...
      const refreshSolanaConfig = async () => {
        if (transaction.version === "legacy") {
          ...
          const dynamicMicroLamports = await _getPriorityFee(serializedTx);
  ...
  }, [setSimulationAccounts, setSolanaFeeConfig, serializedTx, transaction]);
...
async function _getPriorityFee(transaction: string): Promise<number> {
  try {
    ...
    return json.data.solanaPriorityFeeEstimate;
  } catch {
    return 0;
  }
}
