...
export const blockchainExplorerNullable = selectorFamily<
  string | null,
  Blockchain
>({
  key: "blockchainExplorerNullable",
  get:
    (blockchain: Blockchain) =>
    ({ get }) => {
      const p = get(preferencesNullable);
      return p?.blockchains[blockchain].explorer ?? null;
    },
});
...
export const showAllCollectibles = selector<boolean>({
  key: "showAllCollectibles",
  get: ({ get }) => {
    const p = get(preferences);
    return p.showAllCollectibles ?? false;
  },
});
