...
import type { EthereumAsset } from "./utils/EthereumAssetUtils";
import { getEthereumAssetById } from "./utils/EthereumAssetUtils";
...
export class EthereumClient extends BlockchainClientBase<Blockchain.ETHEREUM> {
  ...
  public prefetchedAssets: { [assetId: string]: Promise<EthereumAsset> } = {};

  public prefetchAsset(assetId: string) {
    this.prefetchedAssets[assetId] = getEthereumAssetById(assetId);
  }

  private getAssetById(assetId: string): Promise<EthereumAsset> {
    this.prefetchedAssets[assetId] ??= getEthereumAssetById(assetId);
  ...
  public async transferAsset(
    ...
    if (!asset || asset.__typename === "TokenBalance") {
      if (!asset || asset?.contractAddress === ZeroAddress) {
        ...
        transaction = await Ethereum.transferErc20Transaction(ethereumCtx, {
          to: to.publicKey.toLocaleLowerCase(),
          contractAddress: asset.contractAddress.toLocaleLowerCase(),
          amount: amount.toString(),
        });
      }
    } else if (asset.__typename === "Nft") {
      if (asset.type === "erc1155") {
        transaction = await Ethereum.transferErc1155Transaction(ethereumCtx, {
          ...
          contractAddress: asset.contract.address.toLocaleLowerCase(),
  ...
  public async burnAsset(
    ...
    throw new Error("burnAsset not implemented for Ethereum");
  }
