...
import { metadataAddress } from "../solanaLegacy/programs/token";
import type { RawMintWithProgramIdString } from "../solanaLegacy/types";
...
export const isCardinalWrappedToken = async (
  ...
  return isCardinalWrappedTokenFetch(connection, mintId);
};
export const isCardinalWrappedTokenFetch = async (
  connection: Connection,
  mintId: PublicKey
) => {
