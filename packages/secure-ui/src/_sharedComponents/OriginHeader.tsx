...
export function OriginHeader({ origin }: { origin: SecureEventOrigin }) {
  const siteIcon = useSiteIcon(origin.address);
  return (
    <Entity
      icon={
        <Image
...
export function useSiteIcon(address: string): string {
  return address.startsWith("http") && !address.includes("localhost")
    ? `https://www.google.com/s2/favicons?domain=${address}&sz=180`
    : UNKNOWN_ICON_SRC;
}
