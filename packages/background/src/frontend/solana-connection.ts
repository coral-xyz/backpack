...
async function handleImpl<T = any>(
  ...
  switch (method) {
    ...
    //case SOLANA_CONNECTION_RPC_SIMULATE_TRANSACTION:
    //  return await handleSimulateTransaction(ctx, params[0], params[1], params[2]);
    ...
    //case SOLANA_CONNECTION_RPC_SEND_TRANSACTION:
    //  return await handleSendTransaction(ctx, params[0], params[1], params[2]);
...
async function handleSendTransaction(
  ...
  const resp = await ctx.backend.sendTransaction(transaction, signers, options);
  return [resp];
}
*/
