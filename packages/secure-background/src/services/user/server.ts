...
export class UserService {
  ...
  private getUser = async (
    uuid?: string
  ): Promise<SecureEvent<"SECURE_USER_GET">["response"]> => {
  ...
  private handlePing: TransportHandler<"SECURE_USER_PING"> =
    async ({ request, event, respond, error }) => {
      ...
      const shouldLock = autoLockSettings.option !== "never" && (
  ...
  private handleGetAllUsersWithAccounts: TransportHandler<"SECURE_USER_GET_ALL_WITH_ACCOUNTS"> =
    async ({ respond, error, event }) => {
      ...
      if (currentUser.keyringStoreState === KeyringStoreState.NeedsOnboarding) {
