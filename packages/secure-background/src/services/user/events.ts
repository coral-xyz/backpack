...
export type SECURE_USER_EVENTS =
  ...
  | "SECURE_USER_LOCK_KEYRING"
  | "SECURE_USER_SET_ACTIVE"
  | "SECURE_USER_SET_WALLET"
  | "SECURE_USER_REMOVE"
  | "SECURE_USER_REMOVE_WALLET"
  | "SECURE_USER_UPDATE"
...
export interface SECURE_USER_PREVIEW_WALLETS
  extends SecureEventBase<"SECURE_USER_PREVIEW_WALLETS"> {
  request: BlockchainWalletPublicKeyRequest;
  response: {
    wallets: {
      type: string;
      walletDescriptors: BlockchainWalletDescriptor[];
    }[];
  };
  uiResponse: {
    walletDescriptors: Omit<
      BlockchainWalletDescriptor<BlockchainWalletDescriptorType.HARDWARE>,
      "imported"
    >[];
    // wallets: {
    //   type: string,
    //   walletDescriptors: Omit<BlockchainWalletDescriptor, "imported">[]
    // }[]
  };
}

export interface SECURE_USER_LOCK_KEYRING
  extends SecureEventBase<"SECURE_USER_LOCK_KEYRING"> {
  request: {};
  response: {
    locked: true;
  };
...
export interface SECURE_USER_REMOVE_WALLET
  extends SecureEventBase<"SECURE_USER_REMOVE_WALLET"> {
  request: {
    uuid: string;
    blockchain: Blockchain;
    publicKey: string;
