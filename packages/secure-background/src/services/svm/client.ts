...
export class SVMClient {
  ...
  public signIn(
    request: SecureEvent<"SECURE_SVM_SIGN_IN">["request"],
    options: SecureRequestOptions<"SECURE_SVM_SIGN_IN"> = {}
  ): Promise<SecureResponse<"SECURE_SVM_SIGN_IN">> {
    return this.client
      .send({
        name: "SECURE_SVM_SIGN_IN",
        request: request,
        ...options,
      })
      .then((response) => {
