...
import { PersistentStorageKeys } from "../../storageKeys";
...
async function getMigration(
  ...
  const data = await storeInterface.db.get<Migration>(
    PersistentStorageKeys.STORE_MIGRATION_KEY
  );
  return data;
}
...
async function setMigration(
  ...
  await storeInterface.db.set(PersistentStorageKeys.STORE_MIGRATION_KEY, m);
...
async function getMigrationLog(
  ...
  return await storeInterface.db.get(
    PersistentStorageKeys.STORE_MIGRATION_LOG_KEY
  );
}
