 ...
 * x - m/44'/60'/x
 */
const legacyEthereum = (index: number) => {
  const coinType = 60 + HARDENING;
  const path = [44 + HARDENING, coinType];
 ...
 * m/44'/60'/0'/x
 */
const legacyLedgerIndexed = (index: number) => {
  const coinType = 60 + HARDENING;
 ...
 * m/44'/60'/x'/0/0
 */
const legacyLedgerLiveAccount = (accountIndex: number) => {
  const coinType = 60 + HARDENING;
...
//
const legacyBip44RecoveryPaths = (bip44CoinType: number) => {
...
//     m/44/501'/{0...n}'/0'
//
const legacyBip44ChangeRecoveryPaths = (bip44CoinType: number) => {
  return [...Array(LOAD_PUBLIC_KEY_AMOUNT).keys()].map((i) =>
    legacyBip44ChangeIndexed(bip44CoinType, i)
  );
};

const getAccountRecoveryPaths = (
  bip44CoinType: number,
  accountIndex: number
) => {
...
const getRecoveryPaths = (bip44CoinType: number, ledger = false) => {
  /**
