...
import { useActiveWallet, useTensorMintData } from "@coral-xyz/recoil";
  ...
  async function listOnTensor() {
    ...
    try {
      navigation.push(Routes.TensorCollectibleActionScreen, {
        ...
        description: edit
          ? "Updating listing on Tensor..."
          : "Creating listing on Tensor",
      });
      // done no errors
    ...
    <YStack flex={1} space="$4">
      <YStack jc="center" f={1} padding="$3" space="$5">
        <StyledText textAlign="center">
          {edit ? "Edit SOL listing to" : "List for SOL"}
        </StyledText>
        <LargeNumericInput
          decimals={9}
          ...
          {tensorMintData?.floorPrice ? (
            <Stack
              ...
              onPress={() => {
                setDecimalValue(
                  (
                    parseFloat(tensorMintData!.floorPrice!) / LAMPORTS_PER_SOL
                  ).toString()
                );
              }}
            >
              <StyledText color="$accentBlue">Floor</StyledText>
            </Stack>
          ) : null}
          {tensorMintData?.topTrait?.price ? (
            <Stack
              backgroundColor="$accentBlueBackground"
              borderRadius="$10"
              paddingVertical="$2.5"
              ...
              onPress={() => {
                setDecimalValue(
                  (
                    tensorMintData!.topTrait!.price / LAMPORTS_PER_SOL
                  ).toString()
                );
              }}
            >
              <StyledText color="$accentBlue">Top Trait</StyledText>
            </Stack>
          ) : null}
        </XStack>
      </YStack>
      <YStack marginHorizontal="$4">
        <TensorListNftCard
          ...
          name={nft.name!}
          price={decimalValue}
          tensorMintData={tensorMintData}
        />
      </YStack>
      <YStack mx="$4" mb={insets.bottom}>
